...
class Partner extends \Timber\Post
{
    ...
    public static function maybe_redirect(): void
    {
        if (is_post_type_archive(self::$names["slug"])) {
            ...
            $url           = $partners_page ? get_permalink($partners_page) : home_url('/');
            wp_redirect($url, 301);
            exit();
        }
        ...
        register_extended_field_group([
            ...
            "fields"   => [
                TrueFalse::make("Pagina visibile", "page_visible")
                    ->stylized()
                    ->default(false)
                    ->helperText("Se attiva, la pagina del partner è accessibile tramite URL diretto. Se disattivata, i visitatori vengono reindirizzati alla homepage e la pagina non viene indicizzata."),
                Url::make("Link alternativo", "url")
                    ->helperText("Se la pagina non è visibile, questo link viene usato nella cella del displayer. Lascia vuoto per non avere nessun link.")
                    ->conditionalLogic([
